# Machine learning for individualised iron supplementation decisions in Brazilian infants: a nationally representative development and validation study with deployment of an interactive clinical tool
...
**Findings**
Iron supplementation was associated with 42% lower anaemia prevalence (OR 0.58, 95% CI 0.40--0.84; p=0.005), 29% lower iron deficiency (OR 0.71, 0.52--0.96; p=0.026), and 36% lower elevated C-reactive protein (OR 0.64, 0.44--0.94; p=0.023). A significant interaction with breastfeeding was identified for haemoglobin (p~interaction~=0.027): iron was associated with +0.43 g/dL higher haemoglobin (p=0.001) in non-breastfed infants but showed no association in breastfed infants (+0.02 g/dL, p=0.89). In exclusively breastfed infants aged 6--11 months, iron supplementation was associated with a 2.3-fold higher prevalence of fever, diarrhoea, and vomiting (OR 2.34, p=0.023; p~interaction~=0.003) and a 2.7-fold higher prevalence of respiratory hospitalisation (OR 2.73, p=0.023). Causal Forest analysis independently identified the same pattern: the estimated conditional average treatment effect (CATE) for haemoglobin was three-fold higher in non-breastfed (+0.106 g/dL, 95% CI 0.093--0.119) than breastfed infants (+0.036, 0.028--0.043), with breastfeeding ranked among the top SHAP features. All four outcomes showed statistically confirmed heterogeneity in estimated associations (GATES p<0.05). Iron supplementation was associated with lower composite infectious morbidity in 75.7% of infants but higher diarrhoea risk in 97.3%, suggesting opposing associations with systemic and intestinal infectious pathways. The estimated ponderal growth association showed an 11-fold equity gradient across wealth quintiles (poorest: CATE +0.056; wealthiest: +0.005, non-significant), with no racial disparities. The clinical decision tool classified 15.2% of infants as likely to benefit, 58.7% as trade-off, and 26.1% as unlikely to benefit.

**Interpretation**
Breastfeeding is the primary modifier of the association between iron supplementation and health outcomes in Brazilian infants -- a finding independently identified by epidemiological interaction analysis and data-driven double machine learning. These associations do not establish causality, but their convergence across analytical frameworks, biological plausibility through the lactoferrin mechanism, consistency with randomised evidence from African settings, and robustness to unmeasured confounding (E-values 2.17--4.12) support their validity as a basis for generating hypotheses for prospective evaluation. The deployed clinical tool enables individualised benefit--harm assessment across four simultaneous outcomes. These findings suggest that universal supplementation programmes may benefit from incorporating breastfeeding status, and that iron supplementation is an equity-positive intervention meriting preservation for socioeconomically disadvantaged populations. Prospective trials stratified by breastfeeding status are warranted.
...
This is, to our knowledge, the first study to apply Causal Forest (double machine learning) to a nationally representative infant nutrition survey to estimate individual-level heterogeneous associations between iron supplementation and four simultaneous outcomes. We demonstrate convergent evidence from two independent analytical frameworks -- traditional epidemiological interaction analysis (p~interaction~=0.027) and data-driven SHAP feature importance -- identifying breastfeeding as the primary modifier of iron's associations. We quantify an 11-fold equity gradient in the estimated growth association across wealth quintiles and deploy a freely available, open-source interactive clinical decision tool that integrates all four outcome dimensions into an individualised benefit--harm assessment.
...
Causal Forest, developed by Athey, Tibshirani, and Wager,^12^ is a machine learning method that estimates heterogeneous treatment-associated differences from observational data under the assumption of conditional ignorability. Unlike conventional subgroup analyses requiring pre-specified interactions, Causal Forest discovers effect modification in a data-driven manner, providing individual-level conditional average treatment effect (CATE) estimates -- which, in a cross-sectional observational setting such as ours, should be interpreted as estimated conditional associations rather than causal effects. This approach has demonstrated utility in clinical medicine^13,14^ but has not been applied in paediatric nutritional epidemiology.

The convergence of a nationally representative dataset containing both biomarker and clinical outcome data, machine learning methods capable of estimating individualised heterogeneous associations, and web-based deployment platforms enabling real-time clinical decision support creates an opportunity to move beyond the current one-size-fits-all paradigm. We aimed to: (1) estimate adjusted associations between iron supplementation and 52 health outcomes in Brazilian infants; (2) test whether breastfeeding status modifies these associations; (3) estimate individual-level heterogeneous associations across four key outcomes using Causal Forest; and (4) develop and deploy an interactive clinical decision tool for individualised supplementation guidance.
...
**Heterogeneous treatment effect estimation.** We applied Causal Forest with double machine learning (CausalForestDML from the econml library)^12^ to estimate individual-level CATEs for each of the four key outcomes. We emphasise that the term "Causal Forest" refers to the statistical method, not to a claim of causal inference from our cross-sectional design; the validity of CATE estimates depends on the assumption of conditional ignorability given the 25 measured covariates, which we cannot verify empirically. We use this framework as a principled approach to heterogeneous treatment effect estimation that provides valid confidence intervals under its assumptions, while acknowledging that unmeasured confounding may bias individual-level estimates. The double machine learning framework uses nuisance models to flexibly adjust for confounders: gradient boosting regressors (200 estimators, maximum depth 4) modelled both the outcome and treatment assignment as functions of covariates, and the causal forest (500 trees, minimum 30 samples per leaf) estimated treatment effects from the residualised outcome and treatment.
...
The Causal Forest model (CausalForestDML), trained on 80% of data (n=1481--2499, depending on outcome) and validated on a 20% holdout set (n=371--625), independently identified the same patterns as the epidemiological analysis through a data-driven approach without pre-specified interactions (table 3).
...
In this nationally representative study of 3127 Brazilian infants, we developed, validated, and deployed a machine learning framework for heterogeneous treatment effect estimation that identifies which infants are most likely to benefit from or be harmed by iron supplementation across four simultaneous health outcomes. Two independent analytical approaches -- traditional epidemiological interaction analysis and data-driven heterogeneous association estimation with SHAP interpretability -- converged on the same finding: breastfeeding is the primary modifier of the association between iron supplementation and health outcomes. We translated these findings into a freely available interactive clinical decision tool that provides individualised supplementation guidance, representing a concrete application of precision public health in a low- and middle-income country setting.
...
The deployed clinical tool represents an advance in translating machine learning for heterogeneous association estimation into clinical practice.^23^ Unlike risk prediction models that estimate the probability of an outcome, our tool estimates the expected association between iron supplementation and four health outcomes for an individual infant, integrating these dimensions into a single recommendation. The deployed gradient boosting surrogates achieve R^2^ 0.96--0.99 against original Causal Forest predictions, meaning the tool faithfully represents the underlying analytical model with negligible information loss. The tool runs entirely client-side (JavaScript), requires no backend server or internet connection after initial loading, and is freely accessible. Decision curve analysis confirmed clinical utility above a threshold of 0.10, and validation against 10 clinical scenarios achieved 80% accuracy. The tool classified 15.2% of infants as likely to benefit (predominantly non-breastfed, older), 58.7% as trade-off, and 26.1% as unlikely to benefit (predominantly breastfed) -- proportions that challenge the current universal approach. We emphasise that this tool is intended for hypothesis generation and clinical research, not for routine clinical decisions, pending prospective validation.
...
This study has several strengths. It uses a large, nationally representative dataset with both biomarker and clinical outcome data, enabling simultaneous benefit--harm assessment across 52 outcomes. The dual analytical framework -- traditional epidemiology confirmed by machine learning for heterogeneous association estimation -- provides convergent evidence stronger than either method alone. The deployment of a freely available clinical tool with validated thresholds translates findings into actionable guidance. The equity analysis addresses a dimension frequently neglected in supplementation studies. Geographical cross-validation across five macroregions demonstrated model stability.

This study also has limitations that warrant transparent discussion. First, the cross-sectional design precludes definitive causal inference. We use Causal Forest as a principled framework for heterogeneous treatment effect estimation under the assumption of conditional ignorability, not as a substitute for randomisation. However, three features of our setting mitigate confounding by indication: the PNSF recommends universal supplementation regardless of iron status, so variation in exposure reflects adherence and healthcare access rather than clinical need; the negative control outcome (accidental hospitalisation) showed identical rates between groups (0.4% versus 0.4%, p=1.00); and E-values of 2.17--4.12 indicate that an unmeasured confounder would need to be associated with both exposure and outcome by a factor of 2.2--4.1 to fully explain our findings, a magnitude that exceeds typical residual confounding in well-adjusted observational studies.

Second, we lack external validation from an independent dataset. ENANI-2019 is the only Brazilian survey combining biomarkers, clinical morbidity data, and supplementation exposure in this age group, precluding external validation at this time. The leave-one-region-out cross-validation (all ATE differences <0.01 across five macroregions representing markedly different socioeconomic and epidemiological contexts) provides the strongest internal evidence of generalisability currently achievable.

Third, calibration of individual-level CATE predictions against observed within-decile treatment effects was poor for haemoglobin and growth (slopes -0.24 to -0.54). As discussed in the Results, this reflects the inherent noise in computing "observed CATEs" from small within-decile subgroups (n=37--62) in cross-sectional data, rather than necessarily indicating poor model performance. The clinical tool uses categorical thresholds rather than continuous CATE values, making directional accuracy more relevant than precise magnitude calibration.

Fourth, diarrhoea showed limited heterogeneity (97.3% positive CATEs), restricting individualisation on this outcome. This near-universality is biologically plausible: iron-induced gut dysbiosis operates via luminal iron excess irrespective of host characteristics.^7,8^ The tool's value lies in integrating diarrhoea risk against the heterogeneous benefits in haemoglobin, growth, and systemic infections.

Fifth, the interaction findings in exclusively breastfed infants aged 6--11 months are based on subgroups with limited sample sizes and wide confidence intervals. The E-value of 4.12 for the strongest interaction (fever/diarrhoea/vomiting, p~interaction~=0.003) indicates substantial robustness, and the independent identification by machine learning SHAP analysis strengthens the finding, but prospective confirmation remains essential.

Sixth, the clinical decision tool requires prospective validation before routine clinical use, in accordance with the DECIDE-AI framework.^20^ We present it as a research tool and hypothesis generator, not as a validated clinical instrument.
...
Iron supplementation in Brazilian infants shows heterogeneous associations with health outcomes in which breastfeeding status is the primary effect modifier -- a finding independently identified by two analytical frameworks. These associations are consistent with the biological role of lactoferrin, align with randomised evidence from African settings, and are robust to unmeasured confounding (E-values 2.17--4.12), but they remain observational and do not establish causality. The deployed interactive clinical tool enables individualised four-outcome benefit--harm assessment in real time. Iron is an equity-positive intervention that is associated with the largest benefits in the poorest infants. These findings support the hypothesis that a transition from universal to breastfeeding-stratified iron supplementation could preserve equity gains while reducing potential harm in exclusively breastfed infants. Prospective randomised trials stratified by breastfeeding status, ideally embedded within the Brazilian primary care system, are needed to confirm these associations and evaluate the clinical utility of the decision tool before guideline changes.
